...
```{r}
 # --- Indices (fix off-by-one) ---
i0 <- 19
n_train <- 672
idx_train <- i0:(i0 + n_train - 1)   # was i0:(i0 + n_train)

# --- Parse times & build training frame ---
# Adjust tz as appropriate for your site (Gaviota Coast -> America/Los_Angeles)
...
ggplot(df_train, aes(time, count)) +
  ...
  labs(
    ...
    caption = "The blue line shows 24h moving average"
  ) +
...
tsplot((df_train$count))
acf((df_train$count))
tsplot(diff(df_train$count))
acf(diff(df_train$count))
```
...
```{r}
y_ts <- ts(df_train$dat_bc, frequency = 24)
best_model <- auto.arima(
  y_ts,
  
  # --- Your constraints ---
  d = 0, D = 0,           # Fix differencing as requested
  max.p = 2, max.q = 2,  # Set max for p,q
  max.P = 2, max.Q = 2,  # Set max for P,Q
  
  # --- Search options ---
  seasonal = TRUE,       # Ensure it searches for SARIMA models
  stepwise = FALSE,      # THIS IS THE KEY: Run a full search, not stepwise
  approximation = FALSE, # For more accurate (but slower) AIC/BIC
  trace = TRUE           # Optional: Shows you all models it's testing
)
